...
export async function initializeVideoSocketServer(io: Server) {
  ...
  videoIo.on("connection", (socket) => {
    ...
    socket.on("joinRoom", ({ roomId, isGroup, name }) => {
      socket.join(roomId);
      socket.data = { roomId, userId };
      // const room = videoIo.adapter.rooms.get(roomId) ?? new Set();
      // [...room].filter(id => id !== socket.id).forEach(id => {
      //   videoIo.to(id).emit("new-peer", { socketId: socket.id, userId });
      // });
      // const name = socket.handshake.query.name as string || userId;
      socket.to(roomId).emit("new-peer", { socketId: socket.id, userId, name });
      console.log("join room", { roomId, userId })
    });

    socket.on("offer", ({ targetSocketId, sdp, name }) => {
      // const name = socket.handshake.query.name as string || userId;
      videoIo.to(targetSocketId).emit("offer", { sdp, from: socket.id, name });
    });
