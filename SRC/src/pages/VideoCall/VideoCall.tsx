...
const ICE_SERVERS = {
  iceServers: [
    { urls: ["stun:stun.l.google.com:19302", "stun:ss-turn1.xirsys.com"] },
    {
      username: "WoYy6uWAwk4D__Bl1G_0Q2UXPnZglntrbo21uIAcktSj_RGKrnY7-7XcMt44TuClAAAAAGiDPoBuZ3V5ZW50YW50aGFuaDA3MDlpdA==",
      credential: "57e09e0c-6930-11f0-a63c-0242ac140004",
      urls: [
...
const VideoCall: React.FC = () => {
  ...
  const name = profileDataLS?.name || '';

  // --- Socket connection ---
  useEffect(() => {
    connectVideoSocket(userId);
    return () => disconnectVideoSocket();
  }, [userId]);
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      socket.emit("joinRoom", { roomId: isGroup ? receiverId : roomId, userId, isGroup, name });
    };
    ...
    socket.on("new-peer", async ({ socketId, name }) => {
      ...
      pc.ontrack = (e) => {
        setRemoteStreams((prev) => ({
          ...prev,
          [socketId]: { stream: e.streams[0], name: name || socketId },
        }));
      };

      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      socket.emit("offer", { targetSocketId: socketId, sdp: offer, name });
    });

    // --- Handle offer ---
    socket.on("offer", async ({ sdp, from, name }) => {
      const pc = new RTCPeerConnection(ICE_SERVERS);
      peerConnections.current[from] = pc;
      ...
      pc.onicecandidate = (e) => {
        if (e.candidate) socket.emit("ice-candidate", { targetSocketId: from, candidate: e.candidate });
      };

      pc.ontrack = (e) => {
        setRemoteStreams((prev) => ({
          ...prev,
          [from]: { stream: e.streams[0], name: name || from },
        }));
      };
  ...
  const renderRemoteVideos = () => {
    ...
    const renderVideoCard = (streamObj: { stream: MediaStream; name: string }, peerId: string) => (
      <div key={peerId} className="w-full h-full bg-black rounded-lg overflow-hidden relative">
        <video
          autoPlay
          playsInline
          ref={(video) => video && (video.srcObject = streamObj.stream)}
          className="w-full h-full object-cover"
        />
        <div className="absolute bottom-1 left-1 text-white bg-black bg-opacity-50 px-1 rounded">
          {streamObj.name}
        </div>
      </div>
    );

    if (count === 1) return <div className="w-full h-full">{renderVideoCard(entries[0][1], entries[0][0])}</div>;
    if (count === 2) return <div className="grid grid-cols-2 gap-2 h-full">{entries.map(([id, s]) => renderVideoCard(s, id))}</div>;
    if (count === 3)
      return (
        <div className="grid grid-rows-2 gap-2 h-full">
          <div className="grid grid-cols-2 gap-2">{entries.slice(0, 2).map(([id, s]) => renderVideoCard(s, id))}</div>
          <div className="flex justify-center h-full">
            <div className="w-1/2 h-full">{renderVideoCard(entries[2][1], entries[2][0])}</div>
          </div>
        </div>
      );
    ...
    // >= 4
    return (
      <div className="grid gap-2 h-full" style={{ gridTemplateColumns: "repeat(2, 1fr)", gridAutoRows: "1fr" }}>
        {entries.map(([id, s]) => renderVideoCard(s, id))}
      </div>
    );
  };
